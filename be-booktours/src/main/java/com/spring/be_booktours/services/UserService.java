...
import com.spring.be_booktours.services.JWTUtils;
...
public class UserService {
    ...
    // public MyResponse<?> UpdateProfile(String email, ChangeProfileRequest
    ...
    // int totalBookingGte2 = users.stream().filter(user ->
    // user.getBookingHistories().size() >= 2).toList().size();
    // // Tính toán số khách hàng chỉ đăng kí 1 lần duy nhất
    // int totalBookingEq1 = users.stream().filter(user ->
    // user.getBookingHistories().size() == 1).toList().size();
    // // Tính toán tỷ lệ quay lại của khách hàng
    // double returnRate = (double) totalBookingGte2 / (totalBookingGte2 +
    // totalBookingEq1) * 100;
